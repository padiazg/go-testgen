...
func checkUserName(want string) checkServiceCreateUserFn {
    ...
    return func(t *testing.T, s *Service, u *userDomain.User, err error) {
...
For methods, the check closure receives the receiver as the second parameter (after `*testing.T`). Free functions and constructors include only `*testing.T` + return types — no receiver parameter.
